...
public class WeatherDecorator implements WeatherData{
	...
	protected double getPromedio() {
		return this.getTemperaturas().stream().mapToDouble(t -> t).sum() / this.getTemperaturas().size();
	}
	
	protected double getTemperaturaMaxima() {
		return this.getTemperaturas().stream().max((t1, t2) -> Double.compare(t1, t2)).orElse(null);
	}
	
	protected double getTemperaturaMinima() {
